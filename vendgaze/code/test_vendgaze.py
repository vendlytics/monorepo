...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description='Head pose estimation using the Vendgaze network.')
    parser.add_argument(
        '--gpu',
        dest='gpu_id',
...
if __name__ == '__main__':
    ...
    for i, (images, labels, cont_labels, name) in enumerate(test_loader):
        ...
        pitch_predicted = torch.sum(
            pitch_predicted * idx_tensor,
            1).cpu() * 3 - 99
        roll_predicted = torch.sum(
            roll_predicted * idx_tensor,
            1).cpu() * 3 - 99
        ...
        if args.save_viz:
            ...
            if args.batch_size == 1:
                error_string = 'y %.2f, p %.2f, r %.2f' % (torch.sum(
                    torch.abs(
                        yaw_predicted - label_yaw)),
                    torch.sum(
                    torch.abs(
                        pitch_predicted - label_pitch)),
                    torch.sum(
                    torch.abs(
                        roll_predicted - label_roll)))
                cv2.putText(
                    cv2_img,
                    error_string,
                    (30,
