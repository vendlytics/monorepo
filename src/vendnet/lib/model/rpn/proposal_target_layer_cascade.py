...
class _ProposalTargetLayer(nn.Module):
    ...
    def __init__(self, nclasses):
        ...
        self.BBOX_NORMALIZE_MEANS = torch.FloatTensor(
            config.TRAIN.BBOX_NORMALIZE_MEANS)
        self.BBOX_NORMALIZE_STDS = torch.FloatTensor(
            config.TRAIN.BBOX_NORMALIZE_STDS)
        self.BBOX_INSIDE_WEIGHTS = torch.FloatTensor(
            config.TRAIN.BBOX_INSIDE_WEIGHTS)
    ...
    def _sample_rois_pytorch(
        ...
        for i in range(batch_size):
            ...
            fg_inds = torch.nonzero(
                max_overlaps[i] >= config.TRAIN.FG_THRESH).view(-1)
