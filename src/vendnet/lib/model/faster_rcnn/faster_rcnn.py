...
class _fasterRCNN(nn.Module):
    ...
    def __init__(self, classes, class_agnostic):
        ...
        self.RCNN_roi_pool = _RoIPooling(
            config.POOLING_SIZE, config.POOLING_SIZE, 1.0 / 16.0)
        self.RCNN_roi_align = RoIAlignAvg(
            config.POOLING_SIZE, config.POOLING_SIZE, 1.0 / 16.0)

        self.grid_size = config.POOLING_SIZE * \
            2 if config.CROP_RESIZE_WITH_MAX_POOL else config.POOLING_SIZE
    ...
    def _init_weights(self):
        ...
        normal_init(self.RCNN_rpn.RPN_cls_score, 0,
                    0.01, config.TRAIN.TRUNCATED)
        normal_init(self.RCNN_rpn.RPN_bbox_pred, 0,
                    0.01, config.TRAIN.TRUNCATED)
